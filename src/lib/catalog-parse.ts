/**
 * Pure catalog workbook parser (no IndexedDB / DOM) — shared by the in-app
 * importer and scripts/make-pilot-pack.mts.
 */
...
type ListResult<T> = { list: T[]; added: number; updated: number; skipped: number };

export type CatalogParseResult = {
  clients: ListResult<CatalogClient>;
  equipment: ListResult<CatalogEquipment>;
};
...
/**
 * Merge a workbook's clients + equipment into existing lists.
...
export function parseCatalogWorkbook(
  wb: XLSX.WorkBook,
  existingClients: CatalogClient[],
  existingEquip: CatalogEquipment[],
  ...
  return { clients: clientsResult, equipment: equipResult };
}
